/**
 * Reading lesson — submit answer, record progress via record_practice_session on server.
 */
(function () {
  const config = document.getElementById("reading-config");
  if (!config) return;
  ...
  const options = document.querySelectorAll(".reading-opt");
  ...
  function showFeedback(correct, correctIndex, xpEarned, alreadyDone) {
    ...
    if (correct) {
      ...
      const optionsList = document.querySelectorAll(".reading-opt");
      if (optionsList[correctIndex]) {
        optionsList[correctIndex].classList.add("border-success/50", "bg-success/10");
      }
      feedbackEl.textContent = alreadyDone
        ? "Not quite — review the passage. (Already completed, no extra XP.)"
